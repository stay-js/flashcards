...
const Sets: React.FC = () => {
  const { data: sets, isLoading, isError, refetch } = trpc.sets.get.useQuery();
      ...
      <main className="mx-auto w-fit p-6">
        ...
        {!isError && sets && (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Link
              href="/create"
              className="grid h-48 w-80 cursor-pointer place-items-center rounded-lg border bg-white shadow-sm"
            >
              <FiPlus size={48} />
            </Link>
            ...
            {sets.map(({ id, name, description }) => (
              <div
                key={id}
                className="flex h-48 w-80 cursor-pointer flex-col justify-between overflow-hidden rounded-lg border bg-white p-4 shadow-sm"
              >
